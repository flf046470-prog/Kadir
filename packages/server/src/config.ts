...
export interface ServerConfig {
   ...
   * comma-separated list of files to read. '' serves a 404.
   *
   * Configured rather than dropped into `publicDir`, because the file is written by
   * `build:quest` / `build:phone` with the signing certificate's SHA-256 in it, and `dist/client`
   * is regenerated by every client build. The certificate fingerprint is not a secret, but it is
   * only knowable once the keystore exists, so it is gitignored and never enters the image —
   * which is why the JSON itself is accepted here, for hosts that have no file to point at.
   */
